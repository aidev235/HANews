...
**主题：** HRT 猜想、Gabor 系统、时频平移、Schwartz 函数、计算机辅助验证。

**发生了什么。** 作者构造了一个非零 Schwartz 函数，使其十二个不同的时频平移线性相关，从而推翻 Heil–Ramanathan–Topiwala 猜想。这逆转了时频分析中一个核心线性无关性问题的预期答案，也促使人们重新把此前的肯定性结果理解为对仍能保持线性无关的构型之刻画。
...
**主题：** Schiffer 猜想、Pompeiu 问题、Laplacian 特征函数、分岔理论、平面区域。

**发生了什么。** 作者构造了无穷多个并非圆盘、却允许边界上为常数的 Neumann 特征函数的光滑平面区域。这推翻了二维 Schiffer 猜想，并通过它与 Pompeiu 问题的紧密联系否定了相应的平面刚性命题。其方法把旋转对称阶数放宽为实参数，利用对分岔支的一致控制，最终恢复整数对称阶数。
...
**主题：** 有限 Gabor 系统、HRT 猜想、Zak 丛、控制分裂、区间算术。

**发生了什么。** 在十二平移的 HRT 反例之后，作者构造了仅含四个 Schwartz 函数时频平移的线性相关系统。由于已知任意非零 $L^2(\mathbb{R})$ 函数至多三个平移所成的系统必定线性无关，论文由此确定四正是最小基数。
...
**主题：** Bourgain–Brezis 估计、Hodge 系统、Beurling–Ahlfors 变换、端点消去、临界函数空间。

**发生了什么。** Arsénio 引入广义 Beurling–Ahlfors 变换的无迹归一化，其符号具有关于反射为奇的标量分量。由此产生的端点消去完成了所有维数和微分形式次数上的 Hilbert 空间型 Bourgain–Brezis 猜想。多线性反射估计还导出临界 Triebel–Lizorkin 与 Besov 结果，以及一列趋近于一的指数所对应的 Sobolev 估计。
...
**主题：** Gabor 框架、Schwartz 窗函数、Feichtinger 代数、格铺砌与堆积、Balian–Low 定理。

**发生了什么。** 作者完整刻画了哪些一般时频格允许 Schwartz 类 Gabor 框架窗函数，以及哪些允许属于 Feichtinger 代数的窗函数。判据被表述为一个几何铺砌与堆积问题；对于可分格以及特定的块零形式，它还给出在时间或频率上具有紧支撑的光滑窗函数。

**评估与局限。** 该结果把 Gabor 分析中的基本存在性问题与格几何联系起来，并导出尖锐的 Balian–Low 推论。这种刻画针对格 Gabor 系统，不应理解为覆盖任意不规则时频集。本报告评估的是窗口内的 v1；手稿仍未经同行评审。
...
**主题：** 逆 Galois 问题、散在单群、Mathieu 群、Belyi 映射、显式多项式。

**发生了什么。** 作者证明 $M_{23}$ 可作为 $\mathbb{Q}$ 上的 Galois 群出现，并给出一个显式的 23 次有理多项式，其分裂域具有该 Galois 群。构造先从一组三个非刚性共轭类及计算构造的 Belyi 映射出发，得到 $\mathbb{Q}(t)$ 的正则 $M_{23}$ 扩张。
...
**主题：** Brauer 群、周期—指数问题、代数几何、Hodge 理论、三维簇。

**发生了什么。** Perry 构造了每个至少三维的光滑射影簇，使其带有违反预期周期—指数上界的 Brauer 类，其中包括周期为二、指数为八的情形。三维构造适用于任意特征零代数闭域，因而也适用于 $\overline{\mathbb{Q}}$；其障碍来自 Hodge 理论。
...
**主题：** 变分法、秩一凸性、拟凸性、Morrey 问题、齐次被积函数。

**发生了什么。** 当 $m$ 足够大时，Cassese 在 $\mathbb{R}^{2\times m}$ 上构造了处处不拟凸的齐次秩一凸被积函数。这一构造在高余维族中给出 Morrey 问题的强否定解，并分离了支配向量值变分问题下半连续性的两个核心凸性概念。

**评估与局限。** “处处不拟凸”比只在一点失效强得多，但维数限定至关重要：手稿没有解决经典二乘二情形，也没有覆盖每个较小的 $m$。作者报告使用 LLM 协助审阅并形式化部分工作；数学构造本身仍仅由一篇未经同行评审的预印本支持。
...
**主题：** AI 生成证明、形式化验证、数学研究工作流、归属、可复现性。

**发生了什么。** Hankook Ilbo 报道了 OpenAI 的主张：其 Astra 系统在数学与理论计算机科学领域取得十项进展。OpenAI 称，模型生成数学论证，人类借助同一模型整理手稿，模型还把结果形式化为 Lean 代码。若各项结果经得起审查，这组材料将表明前沿系统能够参与研究级证明发现和形式化。

**人类引导、自主性与局限。** 发布材料没有披露完整问题池、失败案例、提示词、轨迹、模型配置、编排方式或人类干预的确切细节，因此成功率与自主程度均无法独立衡量。OpenAI 同时是开发者、发布者与主要担保方。Lean 核验的是编码后的命题，而非它们是否等同于历史问题的表述，也不核验原创性、重要性或功劳归属。一项 [Scientific American 审查](https://www.scientificamerican.com/article/openais-latest-math-breakthroughs-commit-research-misconduct-experts-say/)指出两项重点论证存在引用和原创性问题，OpenAI 随后更正了宣传措辞。窗口内的报道确立的是一项影响重大且可检查的研究主张，而不是同行评审，也不是对全部十项进展的独立确认。
